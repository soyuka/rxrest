(function (global, factory) {
	...
	(factory((global.rxrest = global.rxrest || {})));
...
function fetch(input, init, abortCallback) {
    ...
    let req = client[input.method.toLowerCase()](input.url);
    for (let header of input.headers) {
        req.set(header[0], header[1]);
    }
    return fromPromise$1(input.text())
        .flatMap(body => {
        req.send(body);
        return create$1((add, end, error) => {
