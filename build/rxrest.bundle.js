...
var request = module.exports = request_1.bind(null, Request);
...
for (var key in requestBase$$1) {
  Request.prototype[key] = requestBase$$1[key];
}
...
function fetch(input, init, abortCallback) {
    ...
    var req = client[input.method.toLowerCase()](input.url);
...
function objectToMap(map$$1, item) {
    for (var key in item) {
        if (Array.isArray(item[key])) {
            for (var i = 0; i < item[key].length; i++) {
                map$$1.append(key, item[key][i]);
            }
        } else {
            map$$1.append(key, item[key]);
        }
    }
    return map$$1;
}
...
var RxRest$$1 = function () {
    ...
    createClass(RxRest$$1, [{
        ...
        value: function request(method, body) {
            ...
            var stream = of(request).flatMap(this.expandInterceptors(Config.requestInterceptors)).flatMap(function (request) {
                ...
                if (!Array.isArray(body)) {
                    var _ret = function () {
                        ...
                        return {
                            v: create$1(function (add, end, error) {
                                add(item);
                                end(item);
                            })
                        };
                ...
                return of(body).flatMap(_this.expandInterceptors(Config.errorInterceptors)).flatMap(function (body) {
